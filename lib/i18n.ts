export const dictionaries = {
  en: {
    dashboard: "Dashboard",
    financialOverview: "Financial overview for Rajdarbar Convention Hall",
    financialSnapshot: "Financial Snapshot",
    upcomingEvents: "Upcoming Events",
    newEvent: "New Event",
    viewAll: "View All",
    scheduled: "scheduled",
    collected: "collected",
    booking: "Booking",
    receipts: "Receipts",
    settings: "Settings",
    partyName: "Party Name",
    address: "Address",
    mobileNumber: "Mobile Number",
    bookingDate: "Booking Date",
    eventDate: "Event Date",
    guestsCount: "Number of Guests",
    hallCharge: "Hall Charge",
    gasCharge: "Gas Charge",
    catering: "Catering",
    waitstaff: "Waitstaff",
    eventCharge: "Event Charge",
    advancePayment: "Advance Payment",
    totalPaid: "Total Paid",
    totalAmount: "Total Amount",
    remainingBalance: "Remaining Balance",
    save: "Save",
    cancel: "Cancel",
    delete: "Delete",
    confirmDelete: "Are you sure you want to delete this event?",
    yes: "Yes",
    no: "No",
    status: "Status",
    paid: "Paid",
    pending: "Pending",
    actions: "Actions",
    noEvents: "No upcoming events found.",
    createReceipt: "Create Receipt",
    view: "View",
    edit: "Edit",
    quantity: "Quantity",
    rate: "Rate",
    chargeRate: "Charge Rate (per person)",
    costRate: "Cost Rate (per person)",
    total: "Total",
    companyFund: "Company Fund",
    revenue: "Revenue",
    estRevenue: "Est. Revenue",
    expenses: "Expenses",
    netProfit: "Net Profit",
    margin: "Margin",
    activeProjects: "active projects",
    eventProfitability: "Event Profitability",
    recentTransactions: "Recent Transactions",
    transactions: "Transactions",
    monthlyPL: "Monthly P&L",
    signOut: "Sign Out",
    noTransactionsYet: "No transactions yet.",
    noEventsYet: "No events yet.",
    events: "Events",
    rajdarbar: "Rajdarbar",
    conventionHall: "Convention Hall",
    eventType: "Event Type",
    transactionType: "Type",
    date: "Date",
    category: "Category",
    amount: "Amount",
    description: "Description",
    receiptImageOptional: "Receipt Image (Optional)",
    eventName: "Event Name",
    method: "Method",
    event: "Event",
    project: "Project",
    source: "Source",
    optional: "Optional",
    income: "Income",
    expense: "Expense",
    uncategorized: "Uncategorized",
    none: "None",
    receiptLoggedSuccess: "Expense receipt logged successfully!",
    error: "Error",
    unexpectedError: "An unexpected error occurred.",
    logExpenseReceipt: "Log Expense Receipt",
    totalEvents: "Total Events",
    eventsCompleted: "Events Completed"
  },
  bn: {
    dashboard: "ড্যাশবোর্ড",
    financialOverview: "রাজদরবার কনভেনশন হলের আর্থিক সারসংক্ষেপ",
    financialSnapshot: "আর্থিক সারসংক্ষেপ",
    upcomingEvents: "আসন্ন ইভেন্টসমূহ",
    newEvent: "নতুন ইভেন্ট",
    viewAll: "সব দেখুন",
    scheduled: "নির্ধারিত",
    collected: "সংগৃহীত",
    booking: "বুকিং",
    receipts: "রসিদ",
    settings: "সেটিংস",
    partyName: "পার্টির নাম",
    address: "ঠিকানা",
    mobileNumber: "মোবাইল নম্বর",
    bookingDate: "বুকিংয়ের তারিখ",
    eventDate: "ইভেন্টের তারিখ",
    guestsCount: "অতিথির সংখ্যা",
    hallCharge: "হল চার্জ",
    gasCharge: "গ্যাস চার্জ",
    catering: "ক্যাটারিং",
    waitstaff: "ওয়েটস্টাফ (কয়জন বয়)",
    eventCharge: "ইভেন্ট চার্জ",
    advancePayment: "অগ্রিম পেমেন্ট",
    totalPaid: "মোট পরিশোধিত",
    totalAmount: "মোট পরিমাণ",
    remainingBalance: "বাকি টাকা",
    save: "সংরক্ষণ করুন",
    cancel: "বাতিল করুন",
    delete: "মুছে ফেলুন",
    confirmDelete: "আপনি কি নিশ্চিত যে আপনি এই ইভেন্টটি মুছে ফেলতে চান?",
    yes: "হ্যাঁ",
    no: "না",
    status: "স্ট্যাটাস",
    paid: "পরিশোধিত",
    pending: "বাকি",
    actions: "অ্যাকশন",
    noEvents: "কোনো আসন্ন ইভেন্ট পাওয়া যায়নি।",
    createReceipt: "রসিদ তৈরি করুন",
    view: "দেখুন",
    edit: "এডিট করুন",
    quantity: "পরিমাণ",
    rate: "রেট",
    chargeRate: "চার্জ রেট (জনপ্রতি)",
    costRate: "খরচ রেট (জনপ্রতি)",
    total: "মোট",
    companyFund: "কোম্পানি ফান্ড",
    revenue: "আয়",
    estRevenue: "আনুমানিক আয়",
    expenses: "ব্যয়",
    netProfit: "নিট লাভ",
    margin: "মার্জিন",
    activeProjects: "সক্রিয় প্রকল্প",
    eventProfitability: "ইভেন্টের লাভজনকতা",
    recentTransactions: "সাম্প্রতিক লেনদেন",
    transactions: "লেনদেনসমূহ",
    monthlyPL: "মাসিক লাভ-ক্ষতি",
    signOut: "লগ আউট",
    noTransactionsYet: "এখনো কোনো লেনদেন নেই।",
    noEventsYet: "এখনো কোনো ইভেন্ট নেই।",
    events: "ইভেন্টসমূহ",
    rajdarbar: "রাজদরবার",
    conventionHall: "কনভেনশন হল",
    eventType: "ইভেন্টের ধরন",
    transactionType: "ধরন",
    date: "তারিখ",
    category: "বিভাগ",
    amount: "পরিমাণ",
    description: "বিবরণ",
    receiptImageOptional: "রসিদের ছবি (ঐচ্ছিক)",
    eventName: "ইভেন্টের নাম",
    method: "পদ্ধতি",
    event: "ইভেন্ট",
    project: "প্রকল্প",
    source: "উৎস",
    optional: "ঐচ্ছিক",
    income: "আয়",
    expense: "ব্যয়",
    uncategorized: "বিভাগহীন",
    none: "কোনটি নয়",
    receiptLoggedSuccess: "ব্যয়ের রসিদ সফলভাবে লগ করা হয়েছে!",
    error: "ত্রুটি",
    unexpectedError: "একটি অপ্রত্যাশিত ত্রুটি ঘটেছে।",
    logExpenseReceipt: "ব্যয়ের রসিদ লগ করুন",
    totalEvents: "মোট ইভেন্ট",
    eventsCompleted: "সম্পন্ন ইভেন্ট"
  },
};

export type Language = 'en' | 'bn';
export type DictionaryKey = keyof typeof dictionaries['en'];
